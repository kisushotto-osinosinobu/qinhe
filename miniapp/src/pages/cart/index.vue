...
import { request,requireLogin,user } from '@/api/http'
...
async function submit(){if(!requireLogin()||!list.value.length)return;if(user()?.role!=='MEMBER'){uni.showToast({title:'员工账号不能在小程序下单',icon:'none'});return}submitting.value=true;try{const data=await request<any>('/orders','POST',{idempotencyKey:`mini-${Date.now()}-${Math.random().toString(16).slice(2)}`,channel:'MINIAPP',items:list.value.map(x=>({productId:x.id,quantity:x.quantity}))});list.value=[];saveCart([]);uni.showModal({title:'订单已提交',content:`订单号：${data.orderNo}\n请到店付款，本次未发生真实资金交易。`,showCancel:false,success:()=>uni.switchTab({url:'/pages/orders/index'})})}finally{submitting.value=false}}
onShow(reload)
</script>
